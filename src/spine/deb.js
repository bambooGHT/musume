...
class SpineDebug {
  ...
  init () {
    ...
    let list = _ts.list = [
      ...
      ghs = _ts.ghs = {},
      debugGroup = _ts.debugGroup = new Container();

    // 创建图形，并添加到debug容器
    for (let i = 0, len = list.length; i < len; i++) {
      let name = list[i];
      ghs[name] = name === 'bones' ? new Container() : new Graphics();
  ...
  update () {
    ...
    _ts.lineWidth = 3 / Math.abs(_ts.scale);

    // 清除绘制
    for (let key in ghs) {
      if (ghs[key] instanceof Graphics) {
        ghs[key].clear();
      } else if (ghs[key] instanceof Container) {
  ...
  drawRegionAttachments () {
    ...
    for (let i = 0, len = slots.length; i < len; i++) {
      ...
      if (!attachment.updateOffset) { continue; };
  ...
  drawMeshHullAndMeshTriangles () {
    ...
    for (let i = 0, len = slots.length; i < len; i++) {
      ...
      if (!attachment?.vertices) continue;
  ...
  drawClipping () {
    ...
    for (let i = 0, len = slots.length; i < len; i++) {
      ...
      if (!slot.bone.active) continue;
      let attachment = slot.getAttachment();
      if (!attachment?.worldVerticesLength) continue;
  ...
  //   let bounds = new PIXI.SkeletonBounds();
...
Spines.prototype.updateMyDebugGraphics = drawDebug;
Spines.prototype.update = function (dt) {
